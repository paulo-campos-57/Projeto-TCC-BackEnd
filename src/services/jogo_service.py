...
class JogoService:
    ...
    def processar_dia(sessao: SessaoJogo) -> dict:
        ...
        clientes_interessados = max(0, clientes_totais - clientes_perdidos_total)
    ...
    def _delta_preco(preco_jogador: float, nome_bairro: str) -> int:
        ...
        if preco_jogador > (ideal * JogoService.LIMITE_PRECO_ABUSIVO):
            return -5
    ...
    def _delta_receita(receita_jogador: dict, nome_bairro: str) -> int:
        ...
        distancia = sum(
            abs(receita_jogador.get(ing, 0) - ideal.get(ing, 0)) for ing in ideal.keys()
        )
    ...
    def _combinar_deltas(delta_p, delta_r) -> int:
        combinado = (
            delta_p * JogoService.PESO_PRECO + delta_r * JogoService.PESO_RECEITA
        )
    ...
    def _gerar_mensagem(
        ...
        if preco_jogador > (preco_ideal * JogoService.LIMITE_PRECO_ABUSIVO):
            ...
            partes.append(f"Clientes de {nome_bairro} acharam o preço salgado.")
        else:
